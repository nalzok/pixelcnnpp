...
class PixelCNNPP(nn.Module):
    ...
    def __call__(self, images, *, train):
        ...
        dropout = partial(nn.Dropout, rate=self.dropout_p, deterministic=not train)
        ...
        down_right = shift_down(conv_down(kernel_size=(1, 3))(images)) + shift_right(
            conv_down_right(kernel_size=(2, 1))(images)
        )
        ...
        return ConvOneByOne(10 * self.logistic_components, init_scale=0.1)(
            nn.elu(down_right)
        )
...
class ConvDownRight(nn.Module):
    ...
    def __call__(self, inputs):
        k_h, k_w = self.kernel_size
        padding = ((k_h - 1, 0), (k_w - 1, 0))  # Vertical padding  # Horizontal padding
...
def conditional_params_from_outputs(theta, img):
    ...
    *batch, h, w, c = img.shape
    ...
    m, s, t = jnp.moveaxis(
        jnp.reshape(theta, tuple(batch) + (h, w, c, k, 3)), (-2, -1), (-4, 0)
    )
